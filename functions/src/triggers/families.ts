...
import { isDeleteReq, isPostReq } from "../utils/request_utils";
...
const removeMemberFromFamily = onRequest(async (req: Request, res: Response) => {
    ...
    const leaveResponse: boolean = await leaveFamily(body.memberId);
    if (!leaveResponse) {
        res.status(GENERAL_ERROR_CODE).send(GENERAL_ERROR_MESSAGE);
        return;
    }
